...
_TRANSLATION_JSON_PATH = "translation_output.json"

def _write_translation_records_to_json(records: list[dict[str, str]]) -> None:
    """Append mu_id/original/transliterated/translated records to a JSON file
    instead of printing them. Keeps a running list across all batches."""
    existing: list[dict[str, str]] = []
    if os.path.exists(_TRANSLATION_JSON_PATH):
        try:
            with open(_TRANSLATION_JSON_PATH, "r", encoding="utf-8") as f:
    ...
    with open(_TRANSLATION_JSON_PATH, "w", encoding="utf-8") as f:
...
def _extract_json_object(raw_text: str) -> dict[str, Any] | None:
    ...
    # 2. Find bounds of the JSON object
    start = raw_text.find("{")
    end = raw_text.rfind("}")
    ...
    # 3. Iterate backwards from the last '}' to handle trailing garbage text 
    # (like model hallucinations appended right after the JSON block)
    current_end = end
    while current_end > start:
        candidate_str = raw_text[start : current_end + 1]
        try:
            return json.loads(candidate_str)
        except json.JSONDecodeError:
            current_end = raw_text.rfind("}", start, current_end)
...
class GeminiArtifactService:
        ...
        *,
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                print(json.dumps(payload.get("usageMetadata", {}), indent=2))
    ...
    async def _translate_all_turns(self, turns: list[TranscriptTurn]) -> list[TranscriptTurn]:
        ...
        def _build_prompt(batch: list[TranscriptTurn]) -> str:
            ...
            return (
                ...
                "For every input turn: set transliterated to Latin-script romanisation of orig.You must spell out the pronunciation of `orig` using ONLY English/Latin alphabet characters (a-z, A-Z).- EXAMPLE: Input 'ಜೀವನ ಆಧಾರ' MUST become output 'Jeevana Aadhara'.\n "
            ...
            try:
                ...
                if parsed:
                    ...
                    for item in items:
                     ...
                     # --- print/log this batch's results ---
                   # --- write this batch's results to a JSON file ---
                    batch_records = []
                    for turn_obj in batch:
                        res = gemini_results.get(turn_obj.mu_id)
                        if res:
                            batch_records.append({
                                "mu_id": turn_obj.mu_id,
                                "speaker": turn_obj.speaker,
                                "original": turn_obj.original,
                                "transliterated": res["transliterated"],
                                "translated": res["translated"],
                            })
                    if batch_records:
                        _write_translation_records_to_json(batch_records)
                    # ----------------------------------------
                    ...
                    # --- Robust Fallback for Missing/Mismatched IDs ---
                    unmatched = [t for t in batch if t.mu_id not in gemini_results]
                    if unmatched:
                        logger.warning(
                            f"[{label}] {len(unmatched)}/{len(batch)} turn(s) missing/unmatched from "
                            f"Gemini's response. Applying positional or safe default fallback."
                        )
                        # If Gemini returned a list of items, map them positionally 
                        # to whichever turns are still missing results.
                        available_items = [item for item in items if isinstance(item, dict)]
                        item_idx = 0
                        for turn_obj in batch:
                            if turn_obj.mu_id not in gemini_results:
                                if item_idx < len(available_items):
                                    item = available_items[item_idx]
                                    gemini_results[turn_obj.mu_id] = {
                                        "transliterated": str(item.get("transliterated") or turn_obj.transliterated).strip(),
                                        "translated": str(item.get("translated") or turn_obj.translated).strip(),
                                    }
                                    logger.info(f"[{label}] Positionally recovered turn {turn_obj.mu_id}")
                                else:
                                    # Absolute fallback: preserve original text so the turn never vanishes
                                    gemini_results[turn_obj.mu_id] = {
                                        "transliterated": turn_obj.transliterated or turn_obj.original,
                                        "translated": turn_obj.translated or turn_obj.original,
                                    }
                                item_idx += 1
        ...
        for turn in turns:
            ...
            transliterated = turn.transliterated   # default/fallback, set FIRST


            if not g_translit:
                transliterated = turn.transliterated
                if turn.mu_id in gemini_results:
                    logger.warning(f"[merge] {turn.mu_id}: Gemini returned empty 'transliterated' — using original")
            elif _contains_indic_script(g_translit):
                transliterated = turn.transliterated
                logger.warning(f"[merge] {turn.mu_id}: rejected transliterated (still contains source script) — using original")
            else:
    ...
    async def _generate_summary(self, merged: MergedTranscript, turns: list[TranscriptTurn]) -> tuple[str, list[str]]:
        ...
        try:
            ...
            print(json.dumps(parsed.get("usageMetadata", {}), indent=2))
